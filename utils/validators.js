...
validators.room = function(room)  {
  ...
  } else if ((room = constants.ROOMS[room]) === undefined) {
...
validators.time = function(time) {
  ...
  } else if ((time = constants.TIMES[time]) === undefined) {
...
validators.date = function(date) {
  if (date === null || date === undefined) {
    ...
    return new Error('"' + date + '" is not a valid date. Has to be in ISO 8106-format YYYY-MM-DD. For example "2016-01-15"');
  }
  return date;
}
